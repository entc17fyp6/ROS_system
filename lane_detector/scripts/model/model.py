...
class parsingNet2(torch.nn.Module):
    def __init__(self, size=(288, 800), pretrained=True, backbone='50', cls_dim=(37, 10, 4), use_aux=False, class_nums=7):
        ...
        self.out_fea = self.class_nums*4
        ...
        ###new_edition
        self.fc = torch.nn.Sequential(torch.nn.Linear(115200, 128),
                                 torch.nn.ReLU(),
                                 torch.nn.Dropout(0.1),
                                 torch.nn.Linear(128, self.out_fea))
    ...
    def forward(self, x):
        ...
        fea1 = self.pool(fea).view(-1, 1800)
        group_cls = self.cls(fea1).view(-1, *self.cls_dim)
        fea_ = fea.view(-1,115200)
        ln_mkr = self.fc(fea_).view(-1, *(self.class_nums, 4))
        if self.use_aux:
            return group_cls, aux_seg, ln_mkr

        return group_cls, ln_mkr
